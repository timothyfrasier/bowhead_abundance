...
#                                           #
# REQUIRES: Numbers to be placed in 7 of    #
# the 8 cells of the table.                 #
#############################################
...
infermissing <- function(histcounts) {
    ...
    #--------------------------------------#
    histList = c(histSamp[, "cell123"], histSamp[, "cell12"], histSamp[, "cell13"], histSamp[, "cell23"], histSamp[, "cell1"], histSamp[, "cell2"], histSamp[, "cell3"])
  
  
    #--------------------------------------#
    # Create sighting indexes and add      #
    #--------------------------------------#
    sightingHistories = matrix(0, nrow = nSamp * 7, ncol = 3)
    
    for (i in 1:nSamp) {
      sightingHistories[i, ] = c(2, 2, 2)
    }
    
    for (i in 1:nSamp) {
      sightingHistories[nSamp + i, ] = c(2, 2, 1)
    }
    
    for (i in 1:nSamp) {
      sightingHistories[(2 * nSamp) + i, ] = c(2, 1, 2)
    }
    
    for (i in 1:nSamp) {
      sightingHistories[(3 * nSamp) + i, ] = c(1, 2, 2)
    }
    
    for (i in 1:nSamp) {
      sightingHistories[(4 * nSamp) + i, ] = c(2, 1, 1)
    }
    
    for (i in 1:nSamp) {
      sightingHistories[(5 * nSamp) + i, ] = c(1, 2, 1)
    }
    
    for (i in 1:nSamp) {
      sightingHistories[(6 * nSamp) + i, ] = c(1, 1, 2)
    }
    ...
    modelstring = "
      data {
        ...
        int xMissing[3];         // Vector of data for individuals not seen in any location
      }
      ...
      transformed parameters {
        ...
        for (i in 1:N) {
          mu[i] = exp(b0 + b1[loc1[i]] + b2[loc2[i]] + b3[loc3[i]]);
        }
      }    
      ...
      generated quantities {
        ...
        mu_pred = exp(b0 + b1[xMissing[1]] + b2[xMissing[2]] + b3[xMissing[3]]);
    ...
    #--------------------------#
    print(stanFit)
    print(plot(stanFit, par = c("b0", "b1", "b2", "b3")))
